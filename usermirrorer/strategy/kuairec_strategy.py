...
Country: China
Registered {register_days} days ago
{friend_user_num} Friends | {follow_user_num} Followers | {fans_user_num} Fans
Active Degree: {active_degree}
Role: {role}
"""

template_action_list = """Title:{caption} {category} {cover_text}
Duration: {video_duration}
{play_count} plays | {like_count} likes | {comment_count} comments | {share_count} shares

"""

template_history = """A video watched {time_diff}
Title:{caption} {category} {cover_text}
Duration: {video_duration}
{play_count} plays | {like_count} likes | {comment_count} comments | {share_count} shares
My Behavior: {operation}
...
def feature_action_list(
    ...
    return [{"time_diff": format_time_diff(
        sampled_inter["timestamp"] - process_timestamp(item["item_features"]["upload_dt"])
    )}]
...
def feature_history(
    ...
    """
    try:
        features = [{"operation": str(item["behavior_features"]["operation"])}]
...
# %%
class KuairecDataStrategy(DataStrategy):
