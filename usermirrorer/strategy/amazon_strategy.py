...
from ..formatter.mapping import do_nothing
...
template_action_list = """Product name: {short_title}
Category: {category}
Price: {rev_price}
Ratings: {average_rating} ({rating_number})
"""

template_history = """A product bought {time_diff}
...
def truncate_text(text: str, max_length: int, by_words: bool = False, separator: str = " ") -> str:
    ...
    if by_words:
        ...
        if len(words) <= max_length:
            return text
        return separator.join(words[:max_length]) + '...'
    else:
...
def feature_action_list(
    ...
    """
    return [
        {"short_title": truncate_text(str(item["item_description"]["title"]), 10, by_words=True)},
        {"rev_price": "NA" if pd.isna(item["item_description"]["price"]) or item["item_description"]["price"] == "None" else ("$" + str(item["item_description"]["price"]))},
    ]
...
def feature_history(
    ...
    """
    features = [
        {"rating": str(item["behavior_features"]["rating"])},
        {"review_text": truncate_text(str(item["behavior_features"]["review_text"]), 30, by_words=True)},
        {"review_summary": str(item["behavior_features"]["review_summary"])},
        {"short_title": truncate_text(str(item["item_description"]["title"]), 10, by_words=True)},
        {"rev_price": "NA" if pd.isna(item["item_description"]["price"]) else ("$" + str(item["item_description"]["price"]))},
    ]
    try:
...
# %%
class AmazonDataStrategy(DataStrategy):
